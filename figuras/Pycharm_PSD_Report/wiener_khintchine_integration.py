...
# axis labels
plt.text(t_max, -0.8, r'$t_1$', fontsize=font_size1, ha='right', va='center')
plt.text(ytm, t_max, r'$t_2$', fontsize=font_size1, ha='left', va='top')
...
# others rects labels
x1, b1 = curly_brace(-(tau1-T/2), 0, T/2-0.5, 0.5, position='down', beta=20, step=0.01)
plt.plot(x1, b1, 'k')
plt.text(-(tau1-T/2)/2, T/2-1.6, r"$\tau-\frac{T}{2}$", fontsize=font_size2, ha='center', va='center')
...
ax.annotate(r"$\sqrt{2}(T-\tau)$", xy=(-tau1/2, tau1/2), xycoords='data', xytext=(-2.5, 2), textcoords='data',
            va = "top", ha = "center",
            arrowprops=dict(arrowstyle="->", color="k", shrinkA=10, shrinkB=1, patchA=None,
                            patchB=None, connectionstyle="arc3,rad=0.2"))

plt.plot([-T/2, -T/2+dtau/2], [-T/2+tau1+dtau, -T/2+dtau/2+tau1], 'k')

ax.annotate(r"$\Delta\tau/\sqrt{2}$", xy=((-T+dtau/2)/2, (-T+3*dtau/2+2*tau1)/2), xycoords='data', xytext=(-T+3, 5),
...
plt.axis('off')
plt.savefig('wiener_khintchine_integration.eps', format='eps', bbox_inches='tight')
plt.show()
